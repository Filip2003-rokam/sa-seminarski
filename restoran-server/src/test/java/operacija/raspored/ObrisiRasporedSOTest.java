...
class ObrisiRasporedSOTest {

    private DbRepository broker;
    private ObrisiRasporedSO so;
    ...
    void setUp() throws Exception {
        ...
        so = new ObrisiRasporedSO(broker);
    ...
    @Test
    @DisplayName("Uspešno brisanje rasporeda poziva delete i commit")
    void testUspehBriseRasporedIPotvrdjujeTransakciju() throws Exception {
        so.izvrsi(validanRaspored, null);

        verify(broker).delete(validanRaspored);
        verify(broker).commit();
        verify(broker, never()).rollback();
    }

    @Test
    @DisplayName("Greška brokera pri brisanju rasporeda radi rollback")
    ...
    void testNullParametarBacaGresku() {
        ...
        assertEquals("Sistem nije mogao da obriše raspored (neispravan objekat)!", ex.getMessage());
    }
